...
* **Creation**: Added concise concepts [RAG](/context/rag.md), [Context Engineering](/context/context-engineering.md), [Memory](/agents/memory.md), [Subagents & Multi-Agent](/agents/subagents.md), [Provider-Side Tools](/tools/provider-tools.md), and [Frontend Tools](/tools/frontend-tools.md) (bare definitions to expand later).
...
* **Reorganization**: Renamed `agents/programmatic-tools.md` → [agents/workflows.md](/agents/workflows.md) (retitled "Workflows") and repaired inbound links. Dropped the `ASI-NN` prefix from every `security/threats/` filename and title (e.g. `asi01-agent-goal-hijack.md` → `agent-goal-hijack.md`).
* **Update**: Tightened the [Agent Harness](/agents/harness.md) definition (runtime that drives the model→tool loop) and fixed a YAML-truncated description on [Agent Goal Hijack](/security/threats/agent-goal-hijack.md).
...
* **Creation**: Adopted the [OWASP Agentic Top 10](/security/threats/agent-goal-hijack.md) as `security/threats/` (ASI01–ASI10; the former Prompt Injection concept became [Agent Goal Hijack](/security/threats/agent-goal-hijack.md)); moved [Guardrails](/security/controls/guardrails.md) and [PII Masking](/security/controls/pii-masking.md) to `security/controls/`.
* **Creation**: Scaffolded new concepts — [MCP Gateway](/enterprise/mcp-gateway.md), [deepagents](/agents/deepagents.md), [Agent Harness](/agents/harness.md), [Workflows](/agents/workflows.md), [LangSmith](/observability/langsmith.md), [Langfuse](/observability/langfuse.md), and [LangSmith Evals](/observability/langsmith-evals.md) — frontmatter and cross-links complete, bodies to be filled in.
...
* **Creation**: Added security concepts [Prompt Injection](/security/threats/agent-goal-hijack.md), [Guardrails](/security/controls/guardrails.md), and [PII Masking & Anonymization](/security/controls/pii-masking.md).
